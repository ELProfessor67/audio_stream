...
export const PUT = connectDB(auth(async function (req){
    try{
        const id = req.url.split('/')[6];
        const {name,email,password,permissions,starttime,endtime,djDate,djTimeInDays,djDays, rawTime,timezone,phone,djEventName} = await req.json();
        

        const team = await userModel.findByIdAndUpdate(id,{name,email,djPermissions: permissions,djStartTime:starttime,
            djEndTime: endtime,djDate,djTimeInDays,djDays, rawTime,timezone,phone,djEventName});
            if(password){
