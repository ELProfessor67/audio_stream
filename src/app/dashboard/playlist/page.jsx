...
export default function Page(){
	...
	const deletePlaylist = async (id) => {
		try{
			const {data:rdata} = await axios.delete(`/api/v1/playlist?id=${id}`);
	        			...
	        			<div className="flex items-center gap-4">
	                        ...
	                        <div className="mr-10 flex items-center gap-7">
	                        	...
	                        	<button className="bg-none border-none outline-none" title="view playlist">
	                            	<Link href={`/dashboard/playlist/${data._id}`} className="text-gray-500 flex items-center"><FaArrowUpRightFromSquare size={20}/><span className='ml-3 text-gray-700'>view</span></Link>
	                            </button>
	                       </div>
	        		</div>
